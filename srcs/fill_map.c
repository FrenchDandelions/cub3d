...
static int	checker(char **cub_str, int size, t_cub *cub, char *s)
{
	char	*temp;

	temp = ft_substr_cub3d(s, size, 0, *cub_str);
	if (temp == *cub_str && temp)
		return (cub->flag_double = 1, 2);
	*cub_str = temp;
	if (!*cub_str)
		return (ERR_MALLOC);
	if (!*cub_str[0])
...
static int	fill_texture(char *s, t_cub *cub)
{
	...
	if (ft_strncmp(s, "NO", 2) == 0)
		return (checker(&cub->north_texture, 2, cub, s));
	else if (ft_strncmp(s, "SO", 2) == 0)
		return (checker(&cub->south_texture, 2, cub, s));
	else if (ft_strncmp(s, "WE", 2) == 0)
		return (checker(&cub->west_texture, 2, cub, s));
	else if (ft_strncmp(s, "EA", 2) == 0)
		return (checker(&cub->east_texture, 2, cub, s));
	else if (ft_strncmp(s, "F", 1) == 0)
		return (checker(&cub->floor_color, 1, cub, s));
	else if (ft_strncmp(s, "C", 1) == 0)
		return (checker(&cub->sky_color, 1, cub, s));
	return (2);
}
