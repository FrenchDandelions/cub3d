...
/*   Updated: 2024/05/15 19:46:54 by acroue           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	check_index(char c, t_cub *cub, size_t i, size_t j)
{
	...
	if (c == '2')
	{
		cub->mmap.cl_pk = cub->mmap.color_door_closed;
		set_color(cub, &(cub->img.buffer), (int [2]){i, j});
	}
	if (c == '3')
	{
		cub->mmap.cl_pk = cub->mmap.color_door_open;
		set_color(cub, &(cub->img.buffer), (int [2]){i, j});
	}
	if (c == '4')
	{
		cub->mmap.cl_pk = cub->mmap.color_door_animation;
		set_color(cub, &(cub->img.buffer), (int [2]){i, j});
	}
}
